/**
 * piping_geometries.js — Pure-function factory for every piping type.
 * Optimized polygon densities for ultra-fast startup and smooth 60fps rendering.
 */
(function (root) {
  'use strict';
  ...
  function safeMerge(geos) {
    ...
    const merged = THREE.mergeGeometries(clean, false) || clean[0] || new THREE.SphereGeometry(0.01, 8, 8);
  ...
  const builders = {
    round: (S) => new THREE.SphereGeometry(S * 0.55, 14, 14),

    sphere: (S) => new THREE.SphereGeometry(S * 0.95, 18, 18),
    ...
    rose: (S) => {
      const parts = [];
      const center = new THREE.ConeGeometry(S * 0.34, S * 0.75, 14);
      ...
      for (let ri = 0; ri < rings.length; ri++) {
        ...
        for (let i = 0; i < r.count; i++) {
          ...
          const p = new THREE.SphereGeometry(r.size, 10, 8);
    ...
    dropFlower: (S) => {
      ...
      for (let i = 0; i < petalCount; i++) {
        ...
        const petal = new THREE.SphereGeometry(S * 0.42, 10, 8);
      ...
      for (let i = 0; i < petalCount; i++) {
        ...
        const inner = new THREE.SphereGeometry(S * 0.25, 8, 8);
        inner.scale(0.8, 1.6, 0.3);
        inner.rotateZ(a);
        inner.translate(Math.cos(a) * S * 0.22, Math.sin(a) * S * 0.22, S * 0.12);
        parts.push(inner);
      }
      const core = new THREE.SphereGeometry(S * 0.16, 10, 10);
    ...
    leaf: (S) => {
      ...
      const blade = new THREE.ExtrudeGeometry(shape, {
        ...
        curveSegments: 8, bevelSegments: 1
      });
      blade.center();
      const vein = new THREE.CylinderGeometry(S * 0.035, S * 0.055, S * 2.1, 6);
      vein.translate(0, S * 0.12, S * 0.16);
      const pinch = new THREE.SphereGeometry(S * 0.28, 10, 8);
    ...
    shell: (S) => {
      ...
      const body = new THREE.SphereGeometry(1, 16, 12, 0, Math.PI * 2, 0, Math.PI * 0.5);
      ...
      for (let r = 0; r < ridges; r++) {
        ...
        const tube  = new THREE.TubeGeometry(curve, 14, S * (0.05 + 0.03 * arch), 6, false);
        parts.push(tube);
      }
      return safeMerge(parts);
    },
    ...
    wave: (S) => {
      ...
      parts.push(new THREE.TubeGeometry(mkWave(0, S * 0.02), 48, S * 0.34, 10, false));
      for (let r = 0; r < 5; r++) {
        parts.push(new THREE.TubeGeometry(
          mkWave((r - 2) * S * 0.14, S * (0.28 + r * 0.015)),
          48, S * 0.045, 6, false,
        ));
      }
      const lt = new THREE.ConeGeometry(S * 0.22, S * 0.55, 10);
      lt.rotateZ(Math.PI / 2); lt.translate(-S * 2.2, -S * 0.2, S * 0.04);
      const rt = new THREE.ConeGeometry(S * 0.22, S * 0.55, 10);
    ...
    basketWeave: (S) => {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        parts.push(h);
        const cap = new THREE.CapsuleGeometry(S * 0.11, S * 1.25, 3, 6);
    ...
    frenchLace: (S) => {
      ...
      const mkTube = (pts, r) => parts.push(new THREE.TubeGeometry(
        new THREE.CatmullRomCurve3(pts), 16, r || S * 0.03, 5, false,
      ));
      const mc = new THREE.TorusGeometry(S * 0.82, S * 0.055, 8, 36);
      mc.translate(0, 0, S * 0.03); parts.push(mc);
      const ic = new THREE.TorusGeometry(S * 0.42, S * 0.028, 6, 28);
      ...
      for (let i = 0; i < 8; i++) {
        ...
        const pearl = new THREE.SphereGeometry(S * 0.075, 6, 6);
    ...
    threads: (S) => {
      const p = [];
      for (let i = 0; i < 4; i++) {
        const t = new THREE.CylinderGeometry(S * 0.07, S * 0.07, S * 1.9, 6);
        t.rotateZ(Math.PI / 2);
    ...
    grass: (S) => {
      ...
      for (let i = 0; i < count; i++) {
        ...
        const b = new THREE.CylinderGeometry(S * 0.028, S * 0.045, h, 4);
      ...
      const base = new THREE.CylinderGeometry(S * 0.38, S * 0.4, S * 0.08, 12);
    ...
    heart: (S) => {
      ...
      for (let r = 0; r < ridges; r++) {
        ...
        const tube  = new THREE.TubeGeometry(curve, 36, S * (0.055 + t * 0.055), 6, true);
        parts.push(tube);
      }
      const basePts = [];
      const M = 28;
      ...
      parts.push(new THREE.TubeGeometry(
        new THREE.CatmullRomCurve3(basePts, true), 36, S * 0.04, 5, true,
      ));
      const tip = new THREE.SphereGeometry(S * 0.09, 8, 8);
  ...
  function getGeometry(pool, type, S) {
    ...
    return pool.get(key, () => {
      ...
      if (b) return b(S);
      const g = new THREE.SphereGeometry(S, 14, 14);
